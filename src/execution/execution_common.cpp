...
// auto check_ww_conflict(const TupleMeta &meta, const Tuple &table, RID rid) -> bool {
//   meta.ts_
// }
...
auto UpdateOldUndoLog(const UndoLog &old_log, const Tuple &old_tuple, const Tuple &new_tuple, const Schema *schema,
  ...
  if (old_is_deleted) {
    ...
    return {true, {}, {}, old_log.ts_, old_log.prev_version_};
  }
  if (new_is_deleted) {
    std::vector<bool> modified_fields;
    std::vector<Value> modified_values;
    ...
    while (undo_link->IsValid()) {
      if (undo_link.has_value()) {
        ...
        if (undo_logo.has_value()) {
          ...
          fmt::println(stderr, "-------txn{} {} ts={} is_deleted={}", undo_link.value().prev_txn_ - TXN_START_ID,
